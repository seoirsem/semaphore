...
def return_processed_frame(
    frame, pose, landmarks: bool, bounding_box: bool, show_background, model
):
    """
    ...
    if bounding_box:
        frame = pose.draw_labelled_box(
            frame, "{} conf: {:.1f}%".format(semaphore_letters[idx], 100 * conf)
        )
    else:
        frame = pose.draw_label(
            frame, "{} conf: {:.1f}%".format(semaphore_letters[idx], 100 * conf)
        )
...
def main():
    # nb complexity makes a difference to fidelity. Possible options {0,1,2}
    pose = Pose(complexity=1)
    ...
    if use_camera:
        ...
        video_path = "data/wuthering_heights.mp4"
    ...
    while camera.isOpened():
        ...
        if success:
            frame1, pose = return_processed_frame(
                frame, pose, True, False, False, model
            )
            frame2, pose = return_processed_frame(frame, pose, False, True, True, model)
            x = 1
            frame1 = cv2.resize(
                frame1, (int(frame1.shape[1] // x), int(frame1.shape[0] // x))
            )
            frame2 = cv2.resize(
                frame2, (int(frame2.shape[1] // x), int(frame2.shape[0] // x))
            )
