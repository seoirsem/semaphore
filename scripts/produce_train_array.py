...
def return_real_data_hand_labelled(path: str):
    ...
    for j, f in enumerate(filepaths):
        ...
        if (j + 1) % 20 == 0:
            print(
                "[{}/{}] images processed in {:.1f}s".format(
                    j + 1, count, (time.time() - t1)
                )
            )
            t1 = time.time()
    return df, labels
...
def load_data_df(data_file: str, label_file: str):
    """
...
def plot_random_data(data, labels):
    ...
    d = data.loc[idx]
    label_argmax = np.argmax(labels.loc[idx])
    print(semaphore_letters[label_argmax])
